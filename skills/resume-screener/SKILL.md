...
1. **병기 원칙** - 모든 비교 항목은 열 머리글에 "요구조건"을 작게 표기하고, 셀에는 지원자가 기재한 사실만 넣는다. 표 전체가 "지원자가 이력서에 쓴 내용"이라는 건 상단 안내문 한 번으로 충분히 밝히면 되므로, 셀마다 "지원자 기재:", "기재:" 같은 접두어를 반복해서 붙이지 않는다 - 값만 바로 적는다. 요구조건과 지원자 기재를 한 셀 안에서 최종 판정 기호(O/X, 충족/미충족)로 압축하지도 않는다.
...
8. **특이사항 열** - 앞의 고정 항목(경력·스킬·학력·희망연봉 등)에 안 들어가지만 이력서에 실제로 적혀 있는 사실은 마지막 "특이사항" 열에 넣는다. 예: 포트폴리오/깃허브 링크, 보유 자격증, 경력 공백기간 본인이 직접 언급한 사유, 근무 가능 시작일, 재직 중 여부. 여기도 판단어(우수/특이하게 좋음 등)는 금지 - 이력서에 쓰인 사실만 짧게 옮긴다. 특별히 적을 내용이 없으면 "-"로 둔다(빈칸을 미기재와 혼동하지 않도록 하이픈 사용).
...
<div class="wrap">
  ...
  <table class="compare">
    <thead>
      <tr>
        ...
        <th style="width:20%">지원동기 (지원자 원문 요약)</th>
        <th style="width:12%">특이사항</th>
      </tr>
    </thead>
    <tbody>
      ...
      <tr>
        ...
        <td class="fact">2년</td>
        <td class="fact">Node.js, Express, MySQL, React</td>
        <td class="fact">4년제 컴퓨터공학과 졸업</td>
        <td class="fact">3,800만원</td>
        <td class="fact">"작은 팀에서 프론트-백엔드를 함께 다뤄본 경험을 살려 서비스 전체 흐름에 기여하고 싶다"고 기재</td>
        <td class="fact">깃허브 링크 첨부, 즉시 근무 가능이라고 기재</td>
      </tr>
      <tr>
        <td class="name">이OO</td>
        <td class="fact">5년</td>
        <td class="fact">Node.js, Express, MySQL, AWS(EC2/RDS), TypeScript</td>
        <td class="fact">전문학사 (정보통신과)</td>
        <td class="fact">4,500만원 (희망, 협의 가능이라고 기재)</td>
        <td class="fact">"이전 회사에서 인프라 이관을 주도한 경험을 이어가고 싶다"고 기재</td>
        <td class="fact">현재 재직 중, 근무 가능일은 입사 통보 후 1개월이라고 기재</td>
      </tr>
      <tr>
        <td class="name">박OO</td>
        <td class="fact">1년</td>
        <td class="fact">Node.js, Express / MySQL 기재 없음(스터디 경험만 언급)</td>
  ...
  <p class="footnote">※ "요구조건"란은 채용공고 원문 기준이며, 나머지 열은 지원자가 이력서/지원서에 직접 작성한 내용을 그대로 옮긴 것입니다. 기재가 없는 항목은 "미기재", 특이사항이 없는 경우는 "-"로 표시했습니다. 최종 채용 여부는 작성자(고용주)가 직접 판단합니다.</p>
...
- [ ] 셀 안에 "지원자 기재:", "기재:" 같은 반복 접두어가 남아있지 않은지 (값만 바로 적혀 있는지)
- [ ] 특이사항 열이 판단어 없이 사실만 담고 있는지, 특이사항 없는 지원자는 "-"로 표기했는지
